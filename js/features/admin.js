  ...
  requireTeacherAuth(async (user) => {
    ...
    function renderAiLimitsBody() {
      ...
      box.innerHTML = `
        ...
        <div class="field"><label for="aiLimPdfPages">Số trang đề thi PDF tối đa/lượt (nhận diện bằng AI)</label><input type="number" id="aiLimPdfPages" min="1" step="1" value="${lim.maxPdfPagesForRecognize}" /></div>
      ...
      $('#saveAiLimitsBtn').addEventListener('click', async () => {
        ...
        try {
          const aiLimits = {
            ...
            maxPdfPagesForRecognize: Math.max(1, Number($('#aiLimPdfPages').value) || 1),
            dailyCapByMode: {
