...
(function () {
  requireTeacherAuth(async () => {
    ...
    async function loadExams() {
      ...
      if (!exams.length) { $('#statsExam').innerHTML = '<option value="">Chưa có đề kiểm tra nào</option>'; return; }
      $('#statsExam').innerHTML = exams.map((e) =>
        `<option value="${escapeHtml(e.examId)}">${escapeHtml(e.chapterTitle)} — ${e.questions.length} câu — ${new Date(e.createdAt).toLocaleString('vi-VN')}</option>`
      ).join('');
      await loadResults();
    }
    ...
    async function loadResults() {
      ...
      try {
        ...
        const results = snap.docs.map((d) => d.data())
          .map((s) => ({ studentName: s.studentName, score: s.score, correctCount: s.correctCount, total: s.total, submittedAt: s.submittedAt }))
        ...
        box.innerHTML = `
          <div class="el-info-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:14px;">
            ...
            <div class="info-item"><div class="k">Điểm TB</div><div class="v">${stats.avg}%</div></div>
            <div class="info-item"><div class="k">Cao nhất</div><div class="v">${stats.max}%</div></div>
            <div class="info-item"><div class="k">Thấp nhất</div><div class="v">${stats.min}%</div></div>
          </div>
          ${results.map((r, i) => `
            <div class="quiz-review-item ${r.score >= 70 ? 'ok' : 'bad'}" style="text-align:left;">
              <div class="qi-q">${i + 1}. ${escapeHtml(r.studentName)}</div>
              <div>${r.correctCount}/${r.total} câu đúng — <strong>${r.score}%</strong></div>
              <div class="hint">Nộp lúc: ${new Date(r.submittedAt).toLocaleString('vi-VN')}</div>
            </div>
          `).join('')}
        `;
      } catch (e) {
